...
<xsl:template match="/">|<xsl:apply-templates select="ICD10CM.tabular"/>||</xsl:template>
	
<xsl:template match="ICD10CM.tabular">
	<xsl:call-template name="T-version"></xsl:call-template>
	<xsl:call-template name="T-introduction"></xsl:call-template>
...
<xsl:template name="Node_Format">
	<xsl:param name="Node_Value"/>|<xsl:call-template name="Trim"><xsl:with-param name="s" select="$Node_Value"/>
	</xsl:call-template>
</xsl:template>
